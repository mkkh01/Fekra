...
يستخدم REST عناوين Binance الرسمية بترتيب failover يبدأ بـ`api.binance.com` ثم `data-api.binance.vision`، ويجرب العنوان الثاني عند 418 أو 429 أو timeout قبل تفعيل القاطع. هذا لا يتجاوز حظرًا حقيقيًا إذا رُفض العنوانان، لكنه لم يعد المسار الأساسي. طبقة REST لا تستخدم fallback إلى `curl` بعد فشل HTTP؛ فالفشل الكامل يفعّل circuit breaker ويمنع إعادة الطلبات المتتابعة. لا تُستخدم بيانات قديمة لاتخاذ قرار قد يُبنى عليه تداول حقيقي لاحقًا.
