...
تستخدم IFVG الآن طلب `exchangeInfo` شاملًا واحدًا في بداية كل دورة قرار، بدل طلب endpoint منفصل لكل عملة. لا تُستخدم نتيجة exchangeInfo من دورة سابقة؛ تُعاد قراءتها لكل دورة، وتُشارك فقط بين رموز الدورة نفسها. يوجد قفل يمنع الطلبات المتوازية المكررة، وcooldown مدته 60 ثانية عند فشل endpoint. عند فشل Binance تُسجّل دورة واحدة مجمعة بحالة `EXCHANGE_INFO_UNAVAILABLE` بدل إنشاء عاصفة من الأخطاء لكل رمز. يظهر وقت آخر قراءة وسبب الخطأ ووقت إعادة المحاولة في `market.health_snapshot().exchange_info`.

يستخدم REST الآن عناوين Binance الرسمية بترتيب failover يبدأ بـ`api.binance.com` ثم `data-api.binance.vision`، ويجرب العنوان الثاني عند 418 أو 429 أو timeout قبل تفعيل القاطع. هذا يعالج اختلاف إتاحة العناوين بين بيئات الاستضافة، لكنه لا يتجاوز حظرًا حقيقيًا إذا رُفض العنوانان. عند استمرار الفشل تبقى IFVG موقوفة عن إنشاء صفقات وتظل Weeg معزولة. طبقة REST لا تستخدم fallback إلى `curl` بعد فشل HTTP؛ فالفشل الكامل يفعّل circuit breaker قصيرًا ويمنع إعادة الطلبات المتتابعة. هذا يمنع أن تتحول مشكلة Binance إلى حلقة طلبات من كل رمز، لكنه يعني أن النظام ينتظر عودة endpoint بدل استخدام بيانات قديمة.

## Telegram polling

يُظهر النظام الآن رمز HTTP ووصف Telegram الآمن في سجل polling دون إظهار bot token. حالة `401` أو `403` تعني أن `TELEGRAM_BOT_TOKEN` غير صالح أو غير مقبول، فيتوقف polling بدل تكرار الخطأ بلا نهاية. حالة `409` تعني غالبًا وجود polling آخر لنفس البوت أو تعارضًا مؤقتًا أثناء النشر، فينتظر العامل ثم يعيد المحاولة. حالة `429` تعني حدًا مؤقتًا من Telegram، ويُطبّق العامل انتظارًا أطول. أخطاء الشبكة تستخدم backoff متزايدًا.
