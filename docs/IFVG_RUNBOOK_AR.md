...
هذا يعالج سبب التكرار التشغيلي في السجل، لكنه لا يتجاوز حظرًا حقيقيًا من Binance أو مشكلة اتصال الشبكة؛ عند استمرار الفشل تبقى IFVG موقوفة عن إنشاء صفقات وتظل Weeg معزولة. طبقة REST لا تستخدم fallback إلى `curl` بعد فشل HTTP؛ فـ418/429 أو timeout يفعّل circuit breaker قصيرًا ويمنع إعادة الطلبات المتتابعة. هذا يمنع أن تتحول مشكلة Binance إلى حلقة طلبات من كل رمز، لكنه يعني أن النظام ينتظر عودة endpoint بدل استخدام بيانات قديمة.
