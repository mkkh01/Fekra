...
يحفظ موصل السوق الآن وقت آخر حدث WebSocket ووقت آخر شمعة مغلقة كلٌ على حدة لكل `symbol × interval`. يعتمد عمر الشمعة على وقتها القادم من Binance بعد إغلاقها، وليس على وقت وصول آخر حدث إلى التطبيق. كما يعرض `/api/health` وواجهات السوق مصدر آخر شمعة (`websocket` أو `rest`) وعمر السعر وعمر الشمعة المغلقة.

قبل فتح صفقة آلية جديدة، يتحقق النظام من حداثة `4h` و`1h` و`15m` ومن وجود سعر حي حديث. عند فشل الشرط تُسجل الإشارة كمرشحة محظورة بسبب جودة البيانات ولا تُفتح صفقة جديدة. لا يوقف ذلك مدير الخروج؛ فالصفقات المفتوحة تستمر في المراقبة والإغلاق عند توفر السعر. يعرض health حالة عامة للمقبس وحالة تفصيلية لكل رمز وفاصل، حتى لا يخفي حدث من رمز واحد توقف رمز آخر.
