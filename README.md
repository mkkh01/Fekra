...
يطلب الخادم عقد قرار JSON، ثم يتحقق من `action` ويعيد `WAIT` عند الاستجابة غير المنظمة أو فشل جميع المفاتيح. لا يُقبل `BUY` أو `SELL_REDUCE` ما لم يتضمن الرد `entry_price` و`stop_loss` و`take_profit` صحيحة، ويُحسب العائد/المخاطرة فقط عند وجود هذه المستويات. كل تحليل يدعم البدائل والأدلة المضادة وعدم اليقين، ولا توجد صلاحية تنفيذ حقيقي.
